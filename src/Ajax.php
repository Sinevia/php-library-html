...
class Ajax
{
    ...
    function data($data_type = null)
    {
        if (func_num_args() > 0) {
            ...
            if (is_string($data_type) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>data($data_type)</b>: Parameter <b>$data_type</b> MUST BE of type String(' . implode(", ", $allowed_params) . ') - <b style="color:red">' . (is_object($data_type) ? get_class($data_type) : gettype($data_type)) . '</b> given!');
            }
            if (in_array(strtolower($data_type), $allowed_params) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>in method <b>data($data_type)</b>: Parameter <b>$data_type</b> MUST BE of type String(' . implode(", ", $allowed_params) . ') - <b style="color:red">' . $data_type . '</b> given!');
            }
    ...
    function method($method = null)
    {
        if (func_num_args() > 0) {
            ...
            if (is_string($method) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>$method($method)</b>: Parameter <b>$method</b> MUST BE of type String(' . implode(", ", $allowed_params) . ') - <b style="color:red">' . (is_object($method) ? get_class($method) : gettype($method)) . '</b> given!');
            }
            if (in_array(strtolower($method), $allowed_params) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>in method <b>method($method)</b>: Parameter <b>$method</b> MUST BE of type String(' . implode(", ", $allowed_params) . ') - <b style="color:red">' . $method . '</b> given!', E_USER_ERROR);
            }
    ...
    function url($url = null)
    {
        if (func_num_args() > 0) {
            if (is_string($url) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>url($url)</b>: Argument <b>$url</b> MUST BE of type String - <b style="color:red">' . (is_object($url) ? get_class($url) : gettype($url)) . '</b> given!');
            }
    ...
    function on_success($js = null)
    {
        if (func_num_args() > 0) {
            if (is_string($js) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>on_success($js)</b>: Argument <b>$js</b> MUST BE of type String - <b style="color:red">' . (is_object($js) ? get_class($js) : gettype($js)) . '</b> given!');
            }
    ...
    function on_before_send($js = null)
    {
        if (func_num_args() > 0) {
            if (is_string($js) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>on_before_send($js)</b>: Argument <b>$js</b> MUST BE of type String - <b style="color:red">' . (is_object($js) ? get_class($js) : gettype($js)) . '</b> given!');
            }
    ...
    function on_complete($js = null)
    {
        if (func_num_args() > 0) {
            if (is_string($js) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>on_complete($js)</b>: Argument <b>$js</b> MUST BE of type String - <b style="color:red">' . (is_object($js) ? get_class($js) : gettype($js)) . '</b> given!');
            }
    ...
    function on_error($js = null)
    {
        if (func_num_args() > 0) {
            if (is_string($js) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>on_error($js)</b>: Argument <b>$js</b> MUST BE of type String - <b style="color:red">' . (is_object($js) ? get_class($js) : gettype($js)) . '</b> given!');
            }
    ...
    function timeout($timespan = null)
    {
        if (func_num_args() > 0) {
            if (is_int($timespan) == false) {
                throw new AjaxIllegalArgumentException('In class <b>' . get_class($this) . '</b> in method <b>timeout($timespan)</b>: Argument <b>$timepsan</b> MUST BE Integer - <b style="color:red">' . (is_object($timespan) ? get_class($timespan) : gettype($timespan)) . '</b> given!');
            }
     ...
     * @throws InvalidArgumentException if parameter $widget is not String or S_Widget
    ...
    function update($widget = null)
    {
        if (func_num_args() > 0) {
            if (is_string($widget) == false && ($widget instanceof Element) == false) {
                throw new \InvalidArgumentException('In class <b>' . get_class($this) . '</b> in method <b>update($widget)</b>: Argument <b>$widget</b> MUST BE String or subclass of S_Widget - <b style="color:red">' . (is_object($widget) ? get_class($widget) : gettype($widget)) . '</b> given!');
            }
            ($widget instanceof Element) ? $id = $widget->getId() : $id = $widget;
    ...
    function to_js()
    {
        if ($this->property("url") == null) {
            throw new AjaxNoUrlException('In class <b>' . get_class($this) . '</b> URL must be specified using method <b>url($url)</b>');
        }
        if ($this->property("success") == null && $this->property("replace") == null && $this->property("update") == null && $this->property("dataType") != "script") {
            throw new AjaxNoOnSuccessException('In class <b>' . get_class($this) . '</b> success action must be specified using method <b>on_success($js)</b>');
        }
        ...
        if ($this->property("replace") == null && $this->property("update") == null) {
            ...
            if ($this->property("update") !== null) {
                ...
                // Removing update
                $this->property = \Sinevia\ArrayUtils::keyDelete($this->property, "update");
            }
            // Replace
            else {
                ...
                // Removing replace
                $this->property = \Sinevia\ArrayUtils::array_key_delete($this->property, "replace");
            }
        }
        ...
        if ($this->property("param") !== null) {
            ...
            $data = implode("&", $data);
            $this->property = \Sinevia\ArrayUtils::keyDelete($this->property, "param");
...
// CLASS: S_Ajax                                                             //
//============================== END OF CLASS ===============================//
class AjaxIllegalArgumentException extends \Exception
{
}
class AjaxNoUrlException extends \Exception
{
}
class AjaxNoOnSuccessException extends \Exception
{
}
