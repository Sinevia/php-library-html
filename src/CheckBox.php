...
class CheckBox extends Element
{
    ...
    {
        if (func_num_args() > 0) {
            if (is_bool($is_checked) == false) throw new \InvalidArgumentException('In class <b>' . get_class($this) . '</b> in method <b>checked($is_checked)</b>: Parameter <b>$is_checked</b> MUST BE of type Boolean - <b style="color:red">' . gettype($is_checked) . '</b> given!', E_USER_ERROR);
            $is_checked ? $this->setAttribute('checked', 'checked') : $this->attribute('checked', null);
            return $this;
        } else {
            return ($this->getAttribute('checked') == 'checked') ? true : false;
        }
    }
    //=====================================================================//
    //  METHOD: checked                                                    //
    ...
    {
        if (func_num_args() > 0) {
            if (is_string($value) == false) throw new \InvalidArgumentException('In class <b>' . get_class($this) . '</b> in method <b>value($value)</b>: Parameter <b>$value</b> MUST BE of type String - <b style="color:red">' . (is_object($value) ? get_class($value) : gettype($value)) . '</b> given!');
